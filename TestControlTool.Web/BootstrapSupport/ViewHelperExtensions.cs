...
namespace BootstrapSupport
{
    public static class DefaultScaffoldingExtensions
    {
        ...
        public static PropertyInfo[] VisibleProperties(this IEnumerable Model, IEnumerable<string> notShown)
        {
            ...
            return elementType.GetProperties().Where(info => info.Name != elementType.IdentifierPropertyName() && !notShown.Contains(info.Name)).ToArray();
        }

        public static PropertyInfo[] VisiblePropertiesFromList(this IEnumerable Model, IEnumerable<string> shown)
        {
            var elementType = Model.GetType().GetElementType();
            if (elementType == null)
            {
            ...
            return elementType.GetProperties().Where(info => info.Name != elementType.IdentifierPropertyName() && shown.Contains(info.Name)).ToArray();
        }
        ...
        {
            return model.GetType().GetProperties().Where(info => notShown.Contains(info.Name)).ToArray();
        }
