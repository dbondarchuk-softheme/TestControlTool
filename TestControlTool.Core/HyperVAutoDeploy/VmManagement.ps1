...
function CopyFiles($FileNameWildcards, $SourceFolder, $SourceUserName, $SourcePassword, $RemoteFolder, $RemoteUserName, $RemotePassword, $Timeout = 600)
{
...
function WaitForServiceToStart($MachineName, $UserName, $Password, $ServiceName, $Timeout = 300)
{
	...
	[string]$status = Invoke-Command -Session $session -ScriptBlock {
	    ...
	    while (($service.Status.ToString() -ne "Running") -and ($time -gt 0)) {
